...
Feature: PKV-Agbagedatesatz ändern
  ...
  Scenario: Als Apotheker PKV-Abrechnungsinformationen bereitstellen
    ...
    And TGR find last request to path ".*" with "$.body.message.path.basicPath" matching "/ChargeItem"
    And TGR print current request as rbel-tree
    And TGR print current response as rbel-tree
  ...
  Scenario: Vorbedingung: Als Patient ein IDP Access Token abholen
    Given TGR set default header "X-p12-bytes-base64" to "!{resolve(file('src/test/resources/Patient_AUT_E256.p12.base64'))}"
    And TGR set default header "X-keystore-password" to "00"
    And TGR set default header "X-scope" to "${data.idp.scope}"
    And TGR set default header "X-discovery-document-address" to "${data.idp.discoveryDocumentAddress}"
    And TGR set default header "X-client-id" to "${data.idp.clientId}"
    ...
    And TGR find last request to path "/" with "$..receiver" matching "${data.dockerservices.idp.address}"
    Then TGR current response with attribute "$.responseCode" matches "200"
    And TGR store current response node text value at "$.body" in variable "erp.access_token_patient"
    And TGR store current response node text value at "$.body.body.idNummer" in variable "erp.kvnr"
  ...
  """
    ...
    When TGR send empty GET request to "${data.address_fachdienst_fdv}/ChargeItem/${erp.task_id}"

    And TGR find last request to path "/ChargeItem/${erp.task_id}"
    And TGR print current request as rbel-tree
    And TGR print current response as rbel-tree

    Then TGR current response with attribute "$.responseCode" matches "200"
    And TGR current response with attribute "$.body" does not match "^Error:.*"
    And TGR store current response node text value at "$.body..identifier[?(lowerCase(@.system.value.basicPath) =$ 'accesscode')].value.value" in variable "erp.patient_access_code"

  @AVS
  Scenario: Als Apotheker den geänderten PKV-Abgabedaten signieren
    Given TGR pause test run execution with message "Bitte signieren Sie als Apotheker den geänderten PKV-Abgabedaten auf einer HBA der Generation 2.1."
    Then TGR find last request to path ".*/SignatureService" with "$.header.SOAPAction" matching ".*SignDocument" 
    And TGR print current request as rbel-tree
    And TGR print current response as rbel-tree
    Then TGR current response with attribute "$.responseCode" matches "200"
    And TGR current response with attribute "$.body" does not match "^Error:.*"
  ...
  Scenario: Test: PKV-Abgabedatensatz ändern
    ...
    Given TGR pause test run execution with message "Bitte ändern Sie als Apotheker den PKV-Abgabedatensatz mit dem AccessCode des Patienten: ${erp.patient_access_code}"
    Then TGR find last request to path ".*" with "$.body.message.path.basicPath" matching "/ChargeItem/${erp.task_id}"
    And TGR print current request as rbel-tree
    And TGR print current response as rbel-tree
    Then TGR current response with attribute "$.body.message.responseCode" matches "200"
